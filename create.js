  ...
  downloadBtn.addEventListener('click', () => {
    const crucifixColor = document.getElementById('crucifixColor').value;
    const largeBeadColor = document.getElementById('largeBeadColor').value;
    const smallBeadColor = document.getElementById('smallBeadColor').value;


    // USUNIĘTO String.raw, co pozwala na normalne escapowanie i eliminuje problem z dosłownym traktowaniem backticków
...
width: 36px;
height: 36px;
background-color: ${smallBeadColor};
}

.bead.large {
width: 50px;
height: 50px;
background-color: ${largeBeadColor};
}
...
width: 40px;
height: 120px;
background-color: ${crucifixColor};
position: relative;
border-radius: 6px;
cursor: pointer;
transition: all 0.2s ease-in-out;
...
const prayerSequenceNames = [
  ...
  ...Array.from({length: 5}, (_, i) => [
    'Ojcze Nasz (Tajemnica ' + (i+1) + ')', // Poprawiono: Konkatenacja zamiast Template String
    ...Array(10).fill('Zdrowaś Maryjo (Dziesiątka ' + (i+1) + ')') // Poprawiono: Konkatenacja zamiast Template String
  ]).flat()
];
...
// Poprawiono: Użycie konkatenacji zamiast Template Stringa, aby uniknąć konfliktu backticków
rosaryContainer.style.transform = 'translate(-50%, -50%) translate(' + finalTranslateX + 'px, ' + finalTranslateY + 'px) scale(' + viewScaleFactor + ')';
}
...
</html>`; // <--- Zamykający backtick jest już zwykłym znakiem
